...
###########################################################################################################################################################################
rawMatrix_1 <- read.table("Autosome.FPKM.txt", header=TRUE,   sep="\t", comment.char = "" )  
dim(rawMatrix_1 )
...
AllResults_g <- "ForDeeptools.Filter-Matrix"
if( ! file.exists(AllResults_g) ) { dir.create(path=AllResults_g, recursive = TRUE) }

write.table( myBED ,   file = paste(AllResults_g, "All.Autosome.genomewide-2kb.bed", sep="/"), 
...
##########################
info_1 <- read.table("297Samples_Full_Information.txt", header=TRUE,   sep="\t", comment.char = "" )  
dim(info_1)
info_1[1:10,1:10]
head(info_1)
...
write.table( myBED2 ,   file = paste(AllResults_g, "Kept.Autosome.genomewide-2kb.bed", sep="/"), 
             append = FALSE, quote = FALSE, sep = "\t", eol = "\n", na = "NA", dec = ".",  row.names = F,  col.names = F )
