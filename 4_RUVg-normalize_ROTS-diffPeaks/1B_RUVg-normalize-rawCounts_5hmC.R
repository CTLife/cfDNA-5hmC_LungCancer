...
.libPaths(c("/home/yongpeng/R/x86_64-pc-linux-gnu-library/4.2" , "/home/yongpeng/anaconda3/envs/r4/lib/R/library", "/usr/local/lib/R/site-library", "/usr/lib/R/site-library", "/usr/lib/R/library"))
...
outDir_g = "1B_RUVg-normalize-rawCounts"
...
##############################################################################################################################################################################################
DF_A <- read.table("1A_finalMergedPeaks/5hmC-peaks.merged.rawCounts.bed",  header=TRUE,   sep="\t", comment.char = "" )     
dim( DF_A )
...
##########################
info_1 <- read.table("1A_finalMergedPeaks/Classified_291_samples.txt", header=TRUE,   sep="\t", comment.char = "" )  
dim(info_1)
info_1[1:10,1:4]
## head(info_1)
...
write.table( rawMatrix_4 ,   file = paste(outDir_g, "0.rawCounts.All.info.txt", sep="/"), 
...
group_all <- as.vector( unlist(rawMatrix_4[3, ]) )
group_2   <- as.vector( unlist(rawMatrix_4[5, ]) )
length(group_all)
length(group_2)



rawMatrix_4[1:20,1:5]
myRPKM2 <- matrix(as.numeric(unlist(rawMatrix_4[-c(1:16) , ])), ncol = ncol(rawMatrix_4[-c(1:16), ]))   # Convert to numeric matrix 
myRPKM2[1:5,1:5]
colnames(myRPKM2) = colnames(rawMatrix_4[-c(1:16), ])
rownames(myRPKM2) = rownames(rawMatrix_4[-c(1:16), ])
...
pdf(paste(outDir_g, "1a.RLE.figures_for_rawCounts.pdf", sep="/"), width=20, height=6)
   plotRLE( matrix3, outline=FALSE, ylim=c(-2, 2), col=myColors[group_all] )
dev.off()

pdf(paste(outDir_g, "1b.PCA.figures_for_rawCounts.pdf", sep="/"), width=6, height=6)
    plotPCA(matrix3, col=myColors[group_all], cex=0.5, labels=F , k=3)
    plotPCA(matrix3, col=myColors[group_all], cex=0.5, labels=F   )
    plotPCA(matrix3, col=myColors[group_all], cex=0.5, labels=TRUE)
dev.off()

pdf(paste(outDir_g, "1c.hist.rawCounts.pdf", sep="/"), width=20, height=6)
    hist(  as.vector( matrix3 ), breaks=10000  )
dev.off()
...
pdf( file = paste(outDir_g, "1.HierarchicalClustering.heatmap.pdf", sep="/"),  width=7, height=8  )
...
pdf( file = paste(outDir_g, "1.HierarchicalClustering.heatmap.cluster.pdf", sep="/"),  width=7, height=7  )
...
myHierarchicalClustering_1_g(  mat_3three  = myRPKM2,   path_temp1=paste(outDir_g, "1.HierarchicalClustering", sep="/"),  width1=15, height1=6,  groups1 = group_all  )
###########################################################################################################################################################################
...
NormalizationStep_RUVg <- function(Set,  K=3) {    
  ...
  ## RUVg
  {
    ...
    ## top[nrow(top),]
    ## rownames(top)[1:10]
    empirical  <- rownames(Set)[  which(  ! ( rownames(Set) %in%  rownames(top)[1:(floor(nrow(top) * (95/100) ) )]    ) ) ]   
    length(empirical)
    ## empirical[1:10]
    ...
    ruvg_norm  <- RUVg( Counts , empirical, k=K,  isLog=F )    
    return(ruvg_norm)
  }
}
...
RUVg_norm <- NormalizationStep_RUVg(set3,   K=5)
names(RUVg_norm)


print("RUVg normalization is finished.")
...
rownames(RUVg_matrix2) <-rownames(myRPKM2)
colnames(RUVg_matrix2) <-colnames(myRPKM2)
dim(RUVg_matrix2)

RUVg_matrix2 = rbind( rawMatrix_4[c(1:16), ], RUVg_matrix2 )
...
pdf(paste(outDir_g, "4a.RLE.figures_for_RUVg-normalized.pdf", sep="/"), width=20, height=6)
    plotRLE(RUVg_matrix, outline=FALSE, ylim=c(-2, 2), col=myColors[group_all] , na.rm=TRUE)
dev.off()

pdf(paste(outDir_g, "4b.PCA.figures_for_RUVg-normalized.pdf", sep="/"), width=6, height=6)
    plotPCA(RUVg_matrix, col=myColors[group_all], cex=0.5, labels=F , k=3)
    plotPCA(RUVg_matrix, col=myColors[group_all], cex=0.5, labels=F   )
    plotPCA(RUVg_matrix, col=myColors[group_all], cex=0.5, labels=TRUE)
dev.off()
...
##  myColNames = colnames(RUVg_matrix)
##  bool_1 = ( (myColNames == "Batch1_Ctrl.JK7_S2")  |    (myColNames == "Batch6_Ctrl.CBF81_S9")  |  (myColNames == "Batch6_Ctrl.CBF63_S15")  | (myColNames == "Batch6_Ctrl.CBF49_S1") )
##  length( bool_1 )
##  length( bool_1[bool_1] )
##   
##  RUVg_matrix_1 = RUVg_matrix[, !bool_1]
##  dim(RUVg_matrix_1)


## PCA
library(factoextra)
prcompObj2         <- prcomp(t(RUVg_matrix), scale = F )
prcompObj2_matrix  <- prcompObj2$x
dim(prcompObj2_matrix)
prcompObj2_Contri  <- (prcompObj2$sdev)^2
prcompObj2_Contri  <- prcompObj2_Contri/sum(prcompObj2_Contri)
...
## The function rcorr() [in Hmisc package] can be used to compute the significance levels for pearson and spearman correlations. 
## It returns both the correlation coefficients and the p-value of the correlation for all possible pairs of columns in the data table.
res2 <- rcorr(  as.matrix(RUVg_matrix, type="pearson") )
my_col3=colorRampPalette( c( "cyan4",  "cyan",  "white", "red",   "red4"),  bias = 1,  space = "rgb" )
correlaton_1 = as.matrix( res2$r )
correlaton_1 = round(correlaton_1, digits = 2)

pdf( file = paste(outDir_g, "4.HierarchicalClustering.heatmap.pdf", sep="/"),  width=7, height=8  )
corrplot(correlaton_1, method = "color", type = "full",  title = "", is.corr = FALSE,  order = "original",  tl.col = "black", tl.srt = 90, col = my_col3(100) )
##text(  col(correlaton_1), nrow(correlaton_1)+1-row(correlaton_1),   correlaton_1, cex=1.5)
dev.off() 

pdf( file = paste(outDir_g, "4.HierarchicalClustering.heatmap.cluster.pdf", sep="/"),  width=7, height=7  )
heatmap.2(x= correlaton_1 ,  dendrogram = "both",  scale =  "none" , na.rm=TRUE,  col=my_col3(100), trace = "none",     notecex=1.5, notecol="black", na.color=par("bg") )    
dev.off() 


myHierarchicalClustering_1_g(  mat_3three  = RUVg_matrix,   path_temp1=paste(outDir_g, "4.HierarchicalClustering", sep="/"),  width1=15, height1=6,  groups1 = group_all  )
###########################################################################################################################################################################
...
RUVg_matrix2 <- reset_outliers2(RUVg_matrix)
RUVg_matrix2 <- log2(RUVg_matrix2 + 1)
max(RUVg_matrix2)
min(RUVg_matrix2)
...
## RUVg_matrix2 = myScaleMatrix2(matrix_temp8=RUVg_matrix, upper_temp8 = 1 , lower_temp8 = -1)
...
min(RUVg_matrix2)
dim(RUVg_matrix2)
RUVg_matrix2[1:5,1:5]


## RLE = log-ratio of read count to median read count across sample
pdf(paste(outDir_g, "6a.RLE.figures.pdf", sep="/"), width=20, height=6)
plotRLE( RUVg_matrix2, outline=FALSE, ylim=c(-2, 2), col=myColors[group_all] )
dev.off()

pdf(paste(outDir_g, "6b.PCA.figures.pdf", sep="/"), width=6, height=6)
plotPCA(RUVg_matrix2, col=myColors[group_all], cex=0.5, labels=F , k=3)
...
pdf(paste(outDir_g, "6c.hist.pdf", sep="/"), width=20, height=6)
hist(  as.vector( RUVg_matrix2 ), breaks=100  )
dev.off()


write.table(RUVg_matrix2,    file = paste(outDir_g, "6d.log2(normalized).txt", sep="/"), append = FALSE, quote = FALSE, sep = "\t",
            eol = "\n", na = "NA", dec = ".", row.names = TRUE, col.names = TRUE )
...
## PCA
library(factoextra)
prcompObj2         <- prcomp(t(RUVg_matrix2), scale = F )
prcompObj2_matrix  <- prcompObj2$x
dim(prcompObj2_matrix)
prcompObj2_Contri  <- (prcompObj2$sdev)^2
prcompObj2_Contri  <- prcompObj2_Contri/sum(prcompObj2_Contri)
...
FigureTemp1_2two  <- ggplot( data = dataframeA_2two, aes(x = PC1, y = PC2, shape=as.factor(myType),   color=as.factor(myTech) )) + 
  ...
  MyTheme_1_g( textSize=14,  hjust1=NULL, vjust1=NULL,  angle1=NULL) +
  guides( colour = guide_legend(override.aes = list(size=5)) ,  shape = guide_legend(override.aes = list(size=5)))  
MySaveGgplot2_1_g(ggplot2Figure1=FigureTemp1_2two , path1=outDir_g, fileName1="6.PCA-1", height1=3.5, width1=7)
...
FigureTemp1_2two  <- ggplot( data = dataframeA_2two, aes(x = PC1, y = PC2, shape=as.factor(myType),   color=as.factor(myTech) )) + 
  ...
  MyTheme_1_g( textSize=14,  hjust1=NULL, vjust1=NULL,  angle1=NULL) +
  guides( colour = guide_legend(override.aes = list(size=5)) ,  shape = guide_legend(override.aes = list(size=5)))  
MySaveGgplot2_1_g(ggplot2Figure1=FigureTemp1_2two , path1=outDir_g, fileName1="6.PCA-2", height1=3.5, width1=7)
...
FigureTemp1_2two  <- ggplot( data = dataframeA_2two, aes(x = PC1, y = PC2, shape=as.factor(myType),   color=as.factor(myTech) )) + 
  ...
  MyTheme_1_g( textSize=14,  hjust1=NULL, vjust1=NULL,  angle1=NULL) +
  guides( colour = guide_legend(override.aes = list(size=5)) ,  shape = guide_legend(override.aes = list(size=5)))  
MySaveGgplot2_1_g(ggplot2Figure1=FigureTemp1_2two , path1=outDir_g, fileName1="6.PCA-3", height1=3.5, width1=7)
...
FigureTemp1_2two  <- ggplot( data = dataframeA_2two, aes(x = PC1, y = PC2, shape=as.factor(myType),   color=as.factor(myTech) )) + 
  ...
  MyTheme_1_g( textSize=14,  hjust1=NULL, vjust1=NULL,  angle1=NULL) + ggrepel::geom_text_repel(aes(label = myLabel), size=1) +
  guides( colour = guide_legend(override.aes = list(size=5)) ,  shape = guide_legend(override.aes = list(size=5)))  
MySaveGgplot2_1_g(ggplot2Figure1=FigureTemp1_2two , path1=outDir_g, fileName1="6.PCA-4", height1=3.5, width1=7)
...
## The function rcorr() [in Hmisc package] can be used to compute the significance levels for pearson and spearman correlations. 
## It returns both the correlation coefficients and the p-value of the correlation for all possible pairs of columns in the data table.
res2 <- rcorr(  as.matrix(RUVg_matrix2, type="pearson") )
my_col3=colorRampPalette( c( "cyan4",  "cyan",  "white", "red",   "red4"),  bias = 1,  space = "rgb" )
correlaton_1 = as.matrix( res2$r )
correlaton_1 = round(correlaton_1, digits = 2)

pdf( file = paste(outDir_g, "6.HierarchicalClustering.heatmap.pdf", sep="/"),  width=7, height=8  )
    corrplot(correlaton_1, method = "color", type = "full",  title = "", is.corr = FALSE,  order = "original",  tl.col = "black", tl.srt = 90, col = my_col3(100) )
    ##text(  col(correlaton_1), nrow(correlaton_1)+1-row(correlaton_1),   correlaton_1, cex=1.5)
dev.off() 

pdf( file = paste(outDir_g, "6.HierarchicalClustering.heatmap.cluster.pdf", sep="/"),  width=7, height=7  )
    heatmap.2(x= correlaton_1 ,  dendrogram = "both",  scale =  "none" , na.rm=TRUE,  col=my_col3(100), trace = "none",     notecex=1.5, notecol="black", na.color=par("bg") )    
dev.off() 


myHierarchicalClustering_1_g(  mat_3three  = RUVg_matrix2,   path_temp1=paste(outDir_g, "6.HierarchicalClustering", sep="/"),  width1=15, height1=6,  groups1 = group_all  )
...
RUVg_matrix2 = rbind( rawMatrix_4[c(1:16), ], RUVg_matrix2 )
RUVg_matrix2[1:3, 1:3]
write.table(RUVg_matrix2,    file = paste(outDir_g, "6G.log2.normalizedCounts.withHeader.txt", sep="/"), append = FALSE, quote = FALSE, sep = "\t",
            eol = "\n", na = "NA", dec = ".", row.names = TRUE, col.names = TRUE )
