...
###########################################################################################################################################################################
rawMatrix_1 <- read.table("3c.normalizedCounts.withHeader.txt", header=TRUE,   sep="\t", comment.char = "" )    
dim( rawMatrix_1 )
...
myHeader = rawMatrix_1[c(1:16), ]
myRPKM   = rawMatrix_1[-c(1:16), ]
dim( rawMatrix_1 )
dim( myHeader  )
dim( myRPKM )
## myHeader[,1:3]
...
bool_1 = ( (myColNames == "Batch6_Ctrl.CBF81_S9")  |   (myColNames == "Batch6_Ctrl.CBF63_S15")   |  (myColNames == "Batch6_Ctrl.CBF49_S1") )
length( bool_1 )
length( bool_1[bool_1] )
myHeader = myHeader[, !bool_1]
myRPKM2  = myRPKM2[, !bool_1]
rawMatrix_1 = rawMatrix_1[, !bool_1]


AllResults_g <- "Figures_after-RUVg_remove3samples"
if( ! file.exists(AllResults_g) ) { dir.create(path=AllResults_g, recursive = TRUE) }


write.table(myHeader,    file = paste(AllResults_g, "288samples.matrix.headOnly.txt", sep="/"), 
            append = FALSE, quote = FALSE, sep = "\t", eol = "\n", na = "NA", dec = ".", row.names = TRUE, col.names = TRUE )


write.table(myRPKM2,    file = paste(AllResults_g, "288samples.matrix.valuesOnly.txt", sep="/"), 
            append = FALSE, quote = FALSE, sep = "\t", eol = "\n", na = "NA", dec = ".", row.names = TRUE, col.names = TRUE )


write.table(rawMatrix_1,    file = paste(AllResults_g, "288samples.matrix.txt", sep="/"), 
            append = FALSE, quote = FALSE, sep = "\t", eol = "\n", na = "NA", dec = ".", row.names = TRUE, col.names = TRUE )
...
length(myRPKM2[myRPKM2>11])
length(myRPKM2[myRPKM2 < -2])

myRPKM2[myRPKM2>11] = 11
myRPKM2[myRPKM2 < -2] = -2

dim(myRPKM2)
myRPKM2 = na.omit(myRPKM2) 
dim(myRPKM2)
###########################################################################################################################################################################
...
AllResults_g <- "Figures_after-RUVg_remove3samples/1_batch"
if( ! file.exists(AllResults_g) ) { dir.create(path=AllResults_g, recursive = TRUE) }

my_col_names1 = as.vector(unlist(myHeader[5,] ) )  ## ctrl, unctrl
my_col_names2 = as.vector(unlist(myHeader[3,] ) )  ##### batch
my_col_names3 = as.vector(unlist(myHeader[6,] ) )  ## name for each sample
length( my_col_names1 )
length( my_col_names2 )
length( my_col_names3 )

my_sampleType_temp1 = rep(x= my_col_names1 , each=nrow(myRPKM2) )
my_sampleType_temp2 = rep(x= my_col_names2 , each=nrow(myRPKM2) )
my_sampleType_temp3 = rep(x= my_col_names3 , each=nrow(myRPKM2) )
my_vector_temp      = as.vector( as.matrix( myRPKM2)    )             
...
write.table(cbind(my_vector_temp, my_sampleType_temp1, my_sampleType_temp2, my_sampleType_temp3),    file = paste(AllResults_g, "0.matrix.txt", sep="/"), 
            append = FALSE, quote = FALSE, sep = "\t", eol = "\n", na = "NA", dec = ".", row.names = TRUE, col.names = TRUE )


MyBoxViolinPlot_1_f(vector2=my_vector_temp,   sampleType2=my_sampleType_temp2,  path2=AllResults_g,   fileName2="1.one-feature-as-type",  
                    title2="",  xLab2="Sample types",  yLab2="log2(5hmC level)",    height2=5,   width2=5,   Ymin2= -2, Ymax2=11) 

MyBoxViolinPlot_1_f_each(vector2=my_vector_temp,   sampleType2=my_sampleType_temp3,  path2=AllResults_g,   fileName2="2.each-sample",  
                    title2="",  xLab2="Samples",  yLab2="log2(5hmC level)",    height2=5,   width2=20,   Ymin2= -2, Ymax2=11) 

MyBoxViolinPlot_3_f(vector2=my_vector_temp,   sampleType3= my_sampleType_temp1,  sampleType4= my_sampleType_temp2,  
                    path2=AllResults_g,   fileName2="3a",  
                    title2="",  xLab2="Samples",  yLab2="log2(5hmC level)",    height2=5,   width2=7,   Ymin2= -2, Ymax2=11 )

MyBoxViolinPlot_3_f(vector2=my_vector_temp,   sampleType3= my_sampleType_temp2,  sampleType4= my_sampleType_temp1,  
                    path2=AllResults_g,   fileName2="3b",  
                    title2="",  xLab2="Samples",  yLab2="log2(5hmC level)",    height2=5,   width2=5,   Ymin2= -2, Ymax2=11 )


index_1 = order(my_col_names2)
MyBoxViolinPlot_3A_f(vector2=my_vector_temp,   sampleType3= my_sampleType_temp2,  sampleType4= my_sampleType_temp3,  
                    level_order=my_col_names3[index_1],   path2=AllResults_g,   fileName2="4",  
                    title2="",  xLab2="Samples",  yLab2="log2(5hmC level)",    height2=5,   width2=22,   Ymin2= -2, Ymax2=11 )



## PCA
library(factoextra)
prcompObj2         <- prcomp(t(2^myRPKM2-1), scale = FALSE )
...
write.table(prcompObj2_Contri ,  file = paste(AllResults_g,   "PCA_info_allContribution.txt",  sep="/"), 
            append = FALSE, quote = FALSE, sep = "\t", eol = "\n", na = "NA", dec = ".",  row.names = TRUE,  col.names = TRUE )

dataframeA_2two  <- data.frame( as.data.frame(prcompObj2_matrix),  myTech=my_col_names2, myType=my_col_names1, myLabel=my_col_names3 )
...
FigureTemp1_2two  <- ggplot( data = dataframeA_2two, aes(x = PC1, y = PC2, shape=as.factor(myType),   color=as.factor(myTech) )) + 
  ...
  MyTheme_1_g( textSize=14,  hjust1=NULL, vjust1=NULL,  angle1=NULL) +
  guides( colour = guide_legend(override.aes = list(size=5)) ,  shape = guide_legend(override.aes = list(size=5)))  
MySaveGgplot2_1_g(ggplot2Figure1=FigureTemp1_2two , path1=AllResults_g, fileName1="PCA-1", height1=3.5, width1=7)
...
FigureTemp1_2two  <- ggplot( data = dataframeA_2two, aes(x = PC1, y = PC2, shape=as.factor(myType),   color=as.factor(myTech) )) + 
  ...
  MyTheme_1_g( textSize=14,  hjust1=NULL, vjust1=NULL,  angle1=NULL) +
  guides( colour = guide_legend(override.aes = list(size=5)) ,  shape = guide_legend(override.aes = list(size=5)))  
MySaveGgplot2_1_g(ggplot2Figure1=FigureTemp1_2two , path1=AllResults_g, fileName1="PCA-2", height1=3.5, width1=7)
...
FigureTemp1_2two  <- ggplot( data = dataframeA_2two, aes(x = PC1, y = PC2, shape=as.factor(myType),   color=as.factor(myTech) )) + 
  ...
  MyTheme_1_g( textSize=14,  hjust1=NULL, vjust1=NULL,  angle1=NULL) +
  guides( colour = guide_legend(override.aes = list(size=5)) ,  shape = guide_legend(override.aes = list(size=5)))  
MySaveGgplot2_1_g(ggplot2Figure1=FigureTemp1_2two , path1=AllResults_g, fileName1="PCA-3", height1=3.5, width1=7)
...
FigureTemp1_2two  <- ggplot( data = dataframeA_2two, aes(x = PC1, y = PC2, shape=as.factor(myType),   color=as.factor(myTech) )) + 
  ...
  MyTheme_1_g( textSize=14,  hjust1=NULL, vjust1=NULL,  angle1=NULL) + ggrepel::geom_text_repel(aes(label = myLabel), size=1) +
  guides( colour = guide_legend(override.aes = list(size=5)) ,  shape = guide_legend(override.aes = list(size=5)))  
MySaveGgplot2_1_g(ggplot2Figure1=FigureTemp1_2two , path1=AllResults_g, fileName1="PCA-4", height1=3.5, width1=7)



##### MDS
dataFrame_temp2 <- data.frame( mysex=my_col_names1 ,  mytech=as.vector(my_col_names2) )
MyMultidimensionalScaling_1_g( meLevelMatrix2=(2^myRPKM2-1), path2=paste(AllResults_g, "MDS_Results", sep="/"),  dataFrame_temp2=dataFrame_temp2 )
###########################################################################################################################################################################
...
AllResults_g <- "Figures_after-RUVg_remove3samples/2_disease-state"
if( ! file.exists(AllResults_g) ) { dir.create(path=AllResults_g, recursive = TRUE) }

my_col_names1 = as.vector(unlist(myHeader[5,] ) )  ## ctrl, unctrl
my_col_names2 = as.vector(unlist(myHeader[5,] ) )  ##### disease state
my_col_names3 = as.vector(unlist(myHeader[6,] ) )  ## name for each sample
length( my_col_names1 )
length( my_col_names2 )
length( my_col_names3 )
...
MyBoxViolinPlot_1_f(vector2=my_vector_temp,   sampleType2=my_sampleType_temp2,  path2=AllResults_g,   fileName2="1.one-feature-as-type",  
                    title2="",  xLab2="Sample types",  yLab2="log2(5hmC level)",    height2=5,   width2=4,   Ymin2= -2, Ymax2=11) 
...
MyBoxViolinPlot_3_f(vector2=my_vector_temp,   sampleType3= my_sampleType_temp1,  sampleType4= my_sampleType_temp2,  
                    path2=AllResults_g,   fileName2="3a",  
                    title2="",  xLab2="Samples",  yLab2="log2(5hmC level)",    height2=5,   width2=4,   Ymin2= -2, Ymax2=11 )
...
AllResults_g <- "Figures_after-RUVg_remove3samples/3_Gender"
if( ! file.exists(AllResults_g) ) { dir.create(path=AllResults_g, recursive = TRUE) }

my_col_names1 = as.vector(unlist(myHeader[5,] ) )  ## ctrl, unctrl
my_col_names2 = as.vector(unlist(myHeader[7,] ) )  ##### Gender
my_col_names3 = as.vector(unlist(myHeader[6,] ) )  ## name for each sample
length( my_col_names1 )
length( my_col_names2 )
length( my_col_names3 )
...
AllResults_g <- "Figures_after-RUVg_remove3samples/4_Race"
if( ! file.exists(AllResults_g) ) { dir.create(path=AllResults_g, recursive = TRUE) }

my_col_names1 = as.vector(unlist(myHeader[5,] ) )  ## ctrl, unctrl
my_col_names2 = as.vector(unlist(myHeader[9,] ) )  ##### Race
my_col_names3 = as.vector(unlist(myHeader[6,] ) )  ## name for each sample
length( my_col_names1 )
length( my_col_names2 )
length( my_col_names3 )
...
AllResults_g <- "Figures_after-RUVg_remove3samples/5_Age"
if( ! file.exists(AllResults_g) ) { dir.create(path=AllResults_g, recursive = TRUE) }

my_col_names1 = as.vector(unlist(myHeader[5,] ) )  ## ctrl, unctrl
my_col_names2 = as.vector(unlist(myHeader[11,] ) )  ##### Age
my_col_names3 = as.vector(unlist(myHeader[6,] ) )  ## name for each sample
length( my_col_names1 )
length( my_col_names2 )
length( my_col_names3 )
...
MyBoxViolinPlot_1_f(vector2=my_vector_temp,   sampleType2=my_sampleType_temp2,  path2=AllResults_g,   fileName2="1.one-feature-as-type",  
                    title2="",  xLab2="Sample types",  yLab2="log2(5hmC level)",    height2=5,   width2=15,   Ymin2= -2, Ymax2=11) 
...
MyBoxViolinPlot_3_f(vector2=my_vector_temp,   sampleType3= my_sampleType_temp1,  sampleType4= my_sampleType_temp2,  
                    path2=AllResults_g,   fileName2="3a",  
                    title2="",  xLab2="Samples",  yLab2="log2(5hmC level)",    height2=5,   width2=15,   Ymin2= -2, Ymax2=11 )
...
AllResults_g <- "Figures_after-RUVg_remove3samples/6_Smoking"
if( ! file.exists(AllResults_g) ) { dir.create(path=AllResults_g, recursive = TRUE) }

my_col_names1 = as.vector(unlist(myHeader[5,] ) )  ## ctrl, unctrl
my_col_names2 = as.vector(unlist(myHeader[14,] ) )  ##### Smoking
my_col_names3 = as.vector(unlist(myHeader[6,] ) )  ## name for each sample
length( my_col_names1 )
length( my_col_names2 )
length( my_col_names3 )
...
AllResults_g <- "Figures_after-RUVg_remove3samples/7_EGFRmutation"
if( ! file.exists(AllResults_g) ) { dir.create(path=AllResults_g, recursive = TRUE) }

my_col_names1 = as.vector(unlist(myHeader[5,] ) )  ## ctrl, unctrl
my_col_names2 = as.vector(unlist(myHeader[15,] ) )  ##### EGFRmutation
my_col_names3 = as.vector(unlist(myHeader[6,] ) )  ## name for each sample
length( my_col_names1 )
length( my_col_names2 )
length( my_col_names3 )
