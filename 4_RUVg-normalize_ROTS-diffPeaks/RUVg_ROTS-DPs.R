...
pdf(paste(AllResults_g, "1a.RLE.figures_for_RPKM.pdf", sep="/"), width=20, height=6)
    plotRLE(norm_matrix2, outline=FALSE, ylim=c(-2, 2), col=myColors[myGroups] )
dev.off()

pdf(paste(AllResults_g, "1b.PCA.figures_for_RPKM.pdf", sep="/"), width=6, height=6)
    plotPCA(norm_matrix2, col=myColors[myGroups], cex=0.5, labels=F)
    plotPCA(norm_matrix2, col=myColors[myGroups], cex=0.5, labels=TRUE)
dev.off()
...
## RLE = log-ratio of read count to median read count across sample
pdf(paste(AllResults_g, "2a.RLE.figures_for_log2RPKM.pdf", sep="/"), width=20, height=6)
plotRLE(norm_matrix3, outline=FALSE, ylim=c(-2, 2), col=myColors[myGroups] )
dev.off()

pdf(paste(AllResults_g, "2b.PCA.figures_for_log2RPKM.pdf", sep="/"), width=6, height=6)
plotPCA(norm_matrix3, col=myColors[myGroups], cex=0.5, labels=F)
plotPCA(norm_matrix3, col=myColors[myGroups], cex=0.5, labels=TRUE)
dev.off()

write.table(norm_matrix3,    file = paste(AllResults_g, "2c.log2RPKM.txt", sep="/"), append = FALSE, quote = FALSE, sep = "\t",
            eol = "\n", na = "NA", dec = ".", row.names = TRUE, col.names = TRUE )
