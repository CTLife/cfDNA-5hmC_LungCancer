...
outDir_g = "1B_RUVg-normalizeRPKM"
...
##############################################################################################################################################################################################
rawMatrix_1 <- read.table("1A_RPKM/Kept.info.txt", header=TRUE,   sep="\t", comment.char = "" )    
dim( rawMatrix_1 )
...
group_all <- as.vector( unlist(myHeader[3, ]) )
group_2   <- as.vector( unlist(myHeader[5, ]) )
length(group_all)
length(group_2)
...
RUVg_matrix2 = rbind( myHeader, RUVg_matrix2 )
